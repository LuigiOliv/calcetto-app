...
function AdminPage({ users, setUsers, votes, setVotes, playersWithOverall }) {
    ...
    const handleReopenRegistrations = async (matchId) => {
        ...
        if (match.status === 'CLOSED') {
            ...
            resetData = {
                ...
                goals: [],                          // ← AGGIUNTO: reset goals array
                manualOverride: true,
    ...
    const handleOpenScoreModal = async (matchId) => {
        ...
        // Carica goals esistenti (nuovo formato) o inizializza vuoto
        if (match.goals && match.goals.length > 0) {
            console.log('✅ Carico goals esistenti:', match.goals);
            setMatchGoals(match.goals.map(g => ({ ...g })));
        } else {
    ...
    const handleSaveScore = async () => {
        ...
        try {
            ...
            // Salva array gol (filtra righe vuote)
            const validGoals = matchGoals.filter(g => g.playerId && g.count > 0);
            updates.goals = validGoals;

            // Backward compat: calcola topScorer/topScorerGoals dal nuovo array
            const normalGoals = validGoals.filter(g => !g.isOwnGoal);
            if (normalGoals.length > 0) {
                const top = normalGoals.reduce((best, g) => g.count > best.count ? g : best);
                updates.topScorer = top.playerName;
                updates.topScorerGoals = top.count;
            } else {
                updates.topScorer = null;
                updates.topScorerGoals = null;
            }
                    ...
                    <div className="modal-overlay">
                        <div className="modal-content">
                            ...
                            <div style={{ marginTop: '30px' }}>
                                <h4 style={{ marginBottom: '15px', color: 'var(--volt)' }}>⚽ Gol segnati (opzionale)</h4>

                                {/* Legacy info se partita vecchia senza goals ma con topScorer */}
                                {(!selectedMatchForScore.goals || selectedMatchForScore.goals.length === 0) && selectedMatchForScore.topScorer && matchGoals.length === 0 && (
                                    <div style={{ padding: '8px 12px', background: 'rgba(255,215,0,0.1)', borderRadius: '6px', marginBottom: '12px', fontSize: '13px', color: '#FFD700' }}>
                                        📋 Dato legacy: {selectedMatchForScore.topScorer} ({selectedMatchForScore.topScorerGoals} gol)
                                    </div>
                                )}

                                {matchGoals.map((goal, idx) => {
                                    const allPlayers = [
                                        ...(selectedMatchForScore.teams?.gialli || []),
                                        ...(selectedMatchForScore.teams?.verdi || [])
                                    ];
                                    return (
                                        <div key={idx} style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '8px', flexWrap: 'wrap' }}>
                                            ...
                                            <button
                                                onClick={() => setMatchGoals(matchGoals.filter((_, i) => i !== idx))}
                                ...
                                <button
                                    onClick={() => setMatchGoals([...matchGoals, { playerId: '', playerName: '', count: 1, isOwnGoal: false }])}
                                    style={{ marginTop: '8px', padding: '6px 14px', background: 'var(--bg-card)', border: '1px dashed var(--border)', color: 'var(--volt)', borderRadius: '6px', cursor: 'pointer', fontSize: '13px' }}
                                >
                                    ➕ Aggiungi marcatore
                                </button>

                                {/* Warning somma gol vs risultato */}
                                {scoreGialli && scoreVerdi && matchGoals.length > 0 && (() => {
                                    const gialliIds = (selectedMatchForScore.teams?.gialli || []).map(p => p.playerId);
                                    const verdiIds = (selectedMatchForScore.teams?.verdi || []).map(p => p.playerId);
                                    const validGoals = matchGoals.filter(g => g.playerId && g.count > 0);
                                    const gialliGoalCount = validGoals
                                        .filter(g => (!g.isOwnGoal && gialliIds.includes(g.playerId)) || (g.isOwnGoal && verdiIds.includes(g.playerId)))
                                        .reduce((sum, g) => sum + g.count, 0);
                                    const verdiGoalCount = validGoals
                                        .filter(g => (!g.isOwnGoal && verdiIds.includes(g.playerId)) || (g.isOwnGoal && gialliIds.includes(g.playerId)))
                                        .reduce((sum, g) => sum + g.count, 0);
                                    const gS = parseInt(scoreGialli) || 0;
                                    const vS = parseInt(scoreVerdi) || 0;
                                    if (gialliGoalCount !== gS || verdiGoalCount !== vS) {
                                        return (
                                            <div style={{ marginTop: '10px', padding: '8px 12px', background: 'rgba(229,62,62,0.15)', borderRadius: '6px', fontSize: '13px', color: '#fc8181' }}>
                                                ⚠️ Gol inseriti: Gialli {gialliGoalCount} - {verdiGoalCount} Verdi (risultato: {gS} - {vS})
                                            </div>
                                        );
                                    }
                                    return null;
